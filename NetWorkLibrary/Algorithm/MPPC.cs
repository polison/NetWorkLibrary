...
namespace NetWorkLibrary.Algorithm
{
    public class MPPC
    {
                ...
                if (p < s)
                {
                    if (history[p++] == history[s++] && history[p++] == history[s])
                    {
                        if (history[p++] == history[++s])
                        {
                            ...
                            if (len > 3)
                            {
                                int high = GetHighestBit(len);//len=6.high=2;
                                val =  /*1计算低位数据*/len & (1 << high) - 1  /*1*/| /*2生成固定高位编码*/ ((1 << (high - 1)) - 1) << high + 1 /*2*/;
                                n = high << 1;
                            }
        ...
        {
            int newoff = off | 0x3C0;//1111+6bit =0x0A长度
            int n = 0x0A;
            if (off > 0x3F)
            {
                if (off > 0x13F)
                {
                    newoff = (off - 0x140) | 0xC000;//110+13bit=0x10长度
                    n = 0x10;
                }
                else
                {
                    newoff = (off - 0x40) | 0xE00;//1110+8bit=0x0C长度
                    n = 0x0C;
                }
            }
